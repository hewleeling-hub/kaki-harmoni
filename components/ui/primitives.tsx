...
export function Button({
  ...
  const classes = [
    ...
    "transition duration-200 ease-out disabled:opacity-50 disabled:pointer-events-none",
    VARIANTS[variant],
    SIZES[size],
    full ? "w-full" : "",
    className,
  ...
  const isExternal = external || (href && /^(https?:|tel:|mailto:|https:\/\/wa\.me)/.test(href));
...
export function Card({
  ...
  return (
    <Tag className={`rounded-[22px] border border-line bg-ivory p-6 shadow-[var(--shadow-warm)] ${className}`}>
      {children}
    </Tag>
  );
}
...
export function SectionHeading({
  ...
  return (
    <div className={`${center ? "text-center mx-auto" : ""} max-w-2xl ${className}`}>
      {eyebrow && (
        <p className="mb-2 text-sm font-semibold uppercase tracking-wider text-olive">{eyebrow}</p>
      )}
      <h2 className="text-[26px] leading-tight text-olive-dark sm:text-[30px]">{title}</h2>
